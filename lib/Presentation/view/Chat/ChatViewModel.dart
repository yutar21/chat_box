...
class ChatViewModel extends BaseViewModel<ChatNavigator> {
  ...
  ChatViewModel(this.sendMessageUseCase, this.getMessagesUseCase,
      this.removeUserFromRoomUseCase, this.deleteRoomUseCase);
    ...
    if (messageController.text.isNotEmpty) {
      try {
        ...
        if (e is FirebaseFireStoreDatabaseTimeoutException) {
          ...
          navigator!
              .showFailMessage(message: e.toString(), posActionTitle: "Ok");
        }
      }
    }
  }
  ...
  void showExitRoomQuestionMessage() async {
    navigator!.showQuestionMessage(
        ...
        posActionTitle: "Thoát",
        posAction: exitRoom,
        negativeActionTitle: "Huỷ");
  }

  void exitRoom() async {
    navigator!.showLoading("Đang tải...");
    try {
      var response =
          await removeUserFromRoomUseCase.invoke(room!, provider!.user!.uid);
  ...
  void showDeleteRoomQuestionMessage() async {
    navigator!.showQuestionMessage(
        ...
        posActionTitle: "Xóa bỏ",
        posAction: deleteRoom,
        negativeActionTitle: "Hủy bỏ");
  }
